...
### Key Form State Management Points:
1. **State Structure**
   - `profile_image`: Holds the File object for uploads
   - `profile_image_url`: Stores the URL string
   - `profile_image_upload`: Stores the uploaded file path
   - `profile_image_display`: Controls which image source to show ('url' or 'upload')

2. **State Transitions**
   - When switching to URL: Clear upload path
   - When switching to Upload: Clear URL
   - Always update display preference when changing source

3. **Image Path Construction**
   ```typescript
   // In components
   const imageUrl = formData.profile_image_display === 'url'
     ? formData.profile_image_url
     : formData.profile_image_upload
       ? `${API_URL.replace('/api', '')}/uploads/${formData.profile_image_upload}`
       : '/placeholder.svg';
